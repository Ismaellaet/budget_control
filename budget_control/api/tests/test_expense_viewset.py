...
class TestExpenseViewSet:
    def test_list_expenses(self, api_client_auth, mock_expenses):
        response = api_client_auth.get(reverse("expense-list"))

        assert len(response.data) == len(mock_expenses)
        assert response.status_code == status.HTTP_200_OK

    def test_create_valid_expense(self, api_client_auth, expense_data):
        response = api_client_auth.post(path=reverse("expense-list"), data=expense_data)
    ...
    def test_retrieve_expense(self, api_client_auth):
    ...
    def test_update_expense(self, api_client_auth):
        expense = baker.make("Expense")
    ...
    def test_delete_expense(self, api_client_auth):
        expense = baker.make("Expense")
        url = reverse("expense-detail", args=[expense.id])
    ...
    def test_get_expenses_by_year_month(self, api_client_auth, mock_expenses):
        year, month = 2022, 3
